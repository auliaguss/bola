const renderJadwal = (id, homeTeam, awayTeam, matchday, group, waktu, act) =>{
  ...
  let tombol = "";
  if(act === "delete"){
    tombol = `<a onclick="konfirmasi('${id}', '${homeTeam}', '${awayTeam}', '${matchday}', '${group}', '${waktu}')" id="saveMatch">Delete From The List</a>`
  }
  else{
...
const konfirm = (data) => {
M.toast({
  html: `<span>Are you sure you want to delete?</span>
    <a onclick="deleteMatch('${data}')"
    class="btn-flat toast-action">Yes, remove</a>`});
}